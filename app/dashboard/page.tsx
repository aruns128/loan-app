...
import {fetcher, UnauthorizedError } from "../lib/apiService";

export default function Dashboard() {
  const router = useRouter(); // Initialize useRouter
  const { data: loans = [], error, isLoading } = useSWR('/api/loans', fetcher);
  ...
  if (!loans) return <div className="text-center p-6 text-gray-600">Loading...</div>;

  // Handling error state
  if (error) return <div className="text-center p-6 text-red-600">Error: {error.message}</div>;

  const totalPrincipal = loans.reduce(
    (acc: any, loan: any) => acc + (loan.principal || 0),
    0
  );
  const totalEarned = loans.reduce(
    (acc: any, loan: any) => acc + (loan.earned_amount || 0),
    0
  );

  const top5UpcomingLoans = loans?.filter((loan: any) => loan.status !== "Returned").slice(0, 5);

  const returnedLoans = loans?.filter((loan: any) => loan.status === "Returned");
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <Card className="p-6 shadow-xl rounded-2xl">
            ...
            <ul className="divide-y divide-gray-200">
              {top5UpcomingLoans.map((loan: any, index: Key | null | undefined) => (
